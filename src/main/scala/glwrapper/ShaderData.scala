...
import simplex3d.math.floatx.{Mat4f, Vec2f, Vec3f, Vec4f}
...
object ShaderData {
  ...
  implicit object Mat4fIsShaderData extends ShaderData[Mat4f] {
    def put(data:ByteBuffer, value:Mat4f) = util.putMat4f(data,value)
    def get(data:ByteBuffer, value:Mat4f) = util.getMat4f(data,value)
    def uniform(location:Int, value: Mat4f) {
      val m = util.sharedFloatBuffer(16)
      util.putMat4f(m, value)
      glUniformMatrix4(location, false, m)
    }
    def assign(target:Mat4f, value:Mat4f) = {target := value; target}
    def default = Mat4f(0)
    val byteStride:Int = 64
  }

  implicit object FloatArrayIsShaderData extends ShaderData[Array[Float]] {
    def put(data:ByteBuffer, value:Array[Float]) = {
      require( (data.position & 3) == 0  && (data.limit & 3) == 0, "data needs to be 4 byte aligned")
      val floatData = data.asFloatBuffer()
      floatData.position( data.position >> 2 )
      floatData.limit( data.limit >> 2 )
      require(floatData.limit - floatData.position >= value.length)
      floatData.put(value)
      data.position(floatData.position << 2)
    }

    def get(data:ByteBuffer, value:Array[Float]) = {
      require( (data.position & 3) == 0  && (data.limit & 3) == 0, "data needs to be 4 byte aligned")
      val floatData = data.asFloatBuffer()
      floatData.position( data.position >> 2 )
      floatData.limit( data.limit >> 2 )
    ...
    def uniform(location:Int, value:Array[Float]) = ???
    def assign(target:Array[Float], value:Array[Float]) = {
      require( target.length == value.length )
      Array.copy(value,0,target,0,target.length)
      target
    }
    def default = ???
    val byteStride:Int = ???
